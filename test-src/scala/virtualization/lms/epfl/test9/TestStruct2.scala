...
//
//import java.io.{PrintWriter,StringWriter,FileOutputStream, ByteArrayOutputStream, PrintStream}
//
//
...
//}
//
//trait ComplexStructExp extends ComplexBase with StructExp with Arith {
//
//  def Complex(re: Rep[Double], im: Rep[Double]) = struct[Complex](List("Complex"), Map("re"->re, "im"->im, "abs" -> re))
//  def infix_re(c: Rep[Complex]): Rep[Double] = field[Double](c, "re")
//  def infix_im(c: Rep[Complex]): Rep[Double] = field[Double](c, "im")
//  def infix_abs(c: Rep[Complex]): Rep[Double] = field[Double](c, "abs")//+field[Double](c, "re")
//  
//}
//
//
...
//  
//  trait DSL extends ComplexArith with ArrayLoops with Arith with OrderingOps with Variables with LiftVariables with IfThenElse with RangeOps with Print with StructExp {
...
//      with ScalaGenVariables with ScalaGenIfThenElse with ScalaGenRangeOps  with LivenessOpt
//      with ScalaGenPrint { val IR: self.type = self;
...
//        /*
//         * Needs hybrid approach:
//         * Within a closure or a mapper this approach is fine.
//         * But I need an overview over which fields are alive for this node,
//         * a classic read write scenario (ignore field if it will be overwritten before read,
//         * eliminate field if not read later).
//         * Idea:
//         * - Analysis for each end node going backwards through graph,
//         * and mark for each node which fields are alive at this point.
//         * - Within nodes, reflect only those field changes which are later used.
//         * => Allows creation of narrower subtype and VectorMap to convert to it
//         * => Prevents unnecessary computation
//         * Does not handle write write case yet.
//         * 
//         */
//        def test(x: Rep[Int]) = {
//          type A = {
////            val line : Int
//          }
//          def infix_line(c: Rep[A]): Rep[Int] = field[Int](c, "line")
//          val s = struct[A](List("asdf"), Map("line" -> x, "asDouble" -> x.toDouble))
//          var c = Complex(x.toDouble, 0)
//////          print(c)
////          c = c + Complex(0,x.toDouble)
//          print(c.abs)
//          print(s.line)
////          var t = field[String](s, "line")
////          t = t+"2"
////          print(t)
//          ()
//        }
