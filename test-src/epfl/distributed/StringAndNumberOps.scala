...
trait StringAndNumberOps extends PrimitiveOps with StringOps{
...
//  def infix_%( l : Rep[Long], mod : Rep[Long])(implicit ctx: SourceContext) = long_modulo(l, mod)
...
trait StringAndNumberOpsCodeGen extends ScalaCodegen {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any])(implicit stream: PrintWriter): Unit = rhs match {
    case StringToLong(s) => emitValDef(sym, "java.lang.Long.parseLong(%s)".format(quote(s)))
